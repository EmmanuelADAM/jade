...
- [monitoring](https://github.com/EmmanuelADAM/jade/tree/english/monitoring) : modern, drop-in replacements for the
  historical `jade.tools.DummyAgent.DummyAgent` and `jade.tools.sniffer.Sniffer` windows - a styled message composer
  and a live sniffer with a filterable table and a sequence-diagram view, built with plain Swing (no new dependency).
  Run `monitoring.launch.LaunchModernMonitoringDemo` for a live demo out of the box.
